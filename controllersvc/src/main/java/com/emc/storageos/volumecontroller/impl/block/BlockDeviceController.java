        ...
        if (clone != null) {
            ...
            if (!NullColumnValueGetter.isNullURI(sourceVolume.getConsistencyGroup())) {
                ...
                if (cgId != null) {
                    ...
                    // Does a CG actually exist on the storage system?
                    if (group != null && group.getNameOnStorageSystem(systemURI) != null) {
                        _log.info(String.format("System consistency group %s exists in %s for storage system %s.",
                                group.getNameOnStorageSystem(systemURI), cgId, systemURI));
                        isCG = true;
                    } else {
                        _log.info(String.format("No system consistency group exists in %s for storage system %s.", cgId, systemURI));
                    }
                }
            }
        }
